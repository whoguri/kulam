...
function Item({ isAdmin, isAdvertiser, setSelId, setOpenDeal, e, i }) {
    return <div key={i} className="group relative bg-white overflow-hidden 2xl:px-0 xl:px-0 px-3 text-center rounded-lg">
        ...
        <Link href={`/deals/${e.id}`} >
            {e.image && <Image src={e.image} width={200} height={200} className="group-hover:scale-125 transition-all w-full h-36 object-cover" placeholder="empty" />}
            <div className="group-hover:opacity-100 opacity-0 absolute inset-0 bg-black bg-opacity-70 text-white flex justify-center items-center flex-col transition-all duration-300">
