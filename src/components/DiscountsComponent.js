...
function Item({ isAdmin, isAdvertiser, setSelId, setOpenDiscount, e, i, getList }) {
    let id = e.id
    const [sending, setSending] = useState(false)

    const deletDiscount = async () => {
        try {
            let yes = confirm("Are you sure to delete discount?")
            if (yes) {
                setSending(true)
                let res = await axios.delete(`/api/discounts/${id}`)
    ...
    return <div key={i} className="group relative bg-white overflow-hidden text-center rounded-lg">
        {(isAdmin || isAdvertiser) && <div className="flex items-center gap-2 w-full">
