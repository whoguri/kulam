...
export default function Modal({ onClose, children, title, width, maxWidth, closeButton = true, overlayClass,
    ...
    return (<div className="relative z-[9999]" aria-labelledby="modal-title" role="dialog" aria-modal="true" >
        ...
        <div className="fixed inset-0 z-10 custom-modal flex items-center justify-center">
            <div className="w-full flex justify-center p-4 md:p-0">
                <div ref={ref} className={`custom-modal max-h-[90vh] overflow-hidden scrollbar-none relative transform overflow-y-visible rounded-lg text-left text-lg shadow-xl transition-all ${width ? width : "w-full"} ${maxWidth ? maxWidth : "max-w-[500px]"}  md:my-8 text-start`} style={{ direction: "rtl" }}>
