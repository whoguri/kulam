...
export default function Pagination(props) {
    ...
    return (<nav className="md:px-4 py-4 isolate inline-flex -space-x-px items-center w-full justify-between pt-10" aria-label="Pagination">
        <div className="flex flex-wrap items-center border border-[#080A24] rounded-lg">
            ...
            {range(start, end, 1).map((e, i) => <div key={i}
                ...
                className={`${page === e ? "bg-primary text-white hover:text-white cursor-default" : "hover:bg-white hover:text-background text-pribg-primary"} cursor-pointer font-bold block lg:px-2 text-center lg:py-2 px-1 py-1 text-xs lg:text-sm outline-0`}>
                <div className="md:h-5 md:w-5 h-4 w-auto">{e + 1}
                </div>
            </div>
            )}
            <button type="button" onClick={() => {
            ...
            }} disabled={!(page < parseInt(pageCount))} className="disabled:opacity-60 disabled:pointer-events-none hover:bg-primary hover:text-white cursor-pointer font-medium block lg:px-2 lg:py-2 px-1 py-1 rounded-e-lg text-xs lg:text-sm outline-0">
        ...
        <div>
            <div className="flex items-center gap-2">
                {isHide ? "" : <select value={limit}
                    ...
                    className="text-pribg-primary font-light w-auto block lg:px-2 lg:py-2 px-1 py-1 border border-[#080A24] rounded-lg text-xs lg:text-sm  outline-0">
