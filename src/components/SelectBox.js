export default function SelectBox(props) {
    const { isRequired, value, label, children, formProps, errors, disabled, onChange, defaultOption, clearErrors, selectboxClass } = props
    let error = ""
    if (errors)
    ...
    return (<div className={` ${selectboxClass}`}>
        ...
        <div className="relative">
            {formProps ? <select className={`disabled:bg-gray-200 w-full py-2 px-3 rounded-xl focus-visible:outline-none first-letter:capitalize placeholder:capitalize border border-input text-sm`}
