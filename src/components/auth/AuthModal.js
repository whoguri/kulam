...
function AuthModal({ onClose }) {
    ...
    return (<Modal title="" showHeader={false} width='md:w-2/3 w-[90%]' maxWidth="md:max-w-2xl" onClose={onClose}>
        <div className='md:py-8 py-4'>
            ...
            <div className='flex gap-2 items-center justify-center mb-5 text-xl font-semibold text-background'>
                <button className='relative px-4 pb-1' type='button' onClick={() => { setView("register") }} >Register
                    <div className={`absolute h-0.5 top-full bg-background inset-0 mx-auto transition-all ${view === "register" ? "w-full" : "w-0"}`} />
                </button>
                <button className='relative px-4 pb-1' type='button' onClick={() => { setView("login") }} >Login
                    <div className={`absolute h-0.5 top-full bg-background inset-0 mx-auto transition-all ${view === "login" ? "w-full" : "w-0"}`} />
                </button>
            </div>
            <div>
