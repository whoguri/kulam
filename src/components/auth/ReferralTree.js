...
function ReferralNode({ node, level, index, isLast, open, setOpen }) {
    return (<div onClick={() => {
        ...
        <div className="flex justify-between items-center">
            ...
            <h3 className={`capitalize text-end ${level === 1 ? "" : ""}`}><span className="font-normal text-base">{node.referrals.length > 0 ? <>₪{2000 * node.referrals.length} = {node.referrals.length} *</> : ""} 2000</span> - {node.name} </h3>
        </div>
