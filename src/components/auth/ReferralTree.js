...
function ReferralNode({ node, level, index, isLast, open, setOpen, search }) {
    ...
    }} className={`${referrals.length > 0 ? "cursor-pointer" : ""} ${(open === index && level === 1) ? "text-background bg-gray-50" : (level === 1 ? "hover:text-background" : "")}  ${level === 1 ? "border-b border-x md:px-6 px-4 md:py-6 py-4 text-xl font-bold" : "text-lg font-normal"} ${isLast ? "rounded-b-md" : ""} ${index === 0 ? "border-t rounded-t-md" : ""}`}>
        <div className="flex justify-between items-center">
            <div>
                {(level === 1 && referrals.length > 0) && <button type="button"
                >
