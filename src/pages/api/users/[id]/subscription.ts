...
const subscription_ = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        if (req.method === "POST") {
            const { date, expiry, type, amount } = req.body;
            if (!date || !expiry || !type)
            ...
            await prisma.subscription.updateMany({
                where: {
                    userId: id, isCanceled: false, date: { lte: new Date() },
                    expiry: { gte: new Date() }
                }, data: { isCanceled: true, isAutoRenew: false }
            });
            res.status(201).json(null);
        } else {
...
export default withAdmin(subscription_)